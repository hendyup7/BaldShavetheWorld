...
void Engine::Game::PollInput()
{
	...
	while (SDL_PollEvent(&evt)) {
		...
		case SDL_CONTROLLERBUTTONDOWN:
			joyStickPress(evt.cbutton.button);
			break;
		case SDL_CONTROLLERBUTTONUP:
			joyStickRelease(evt.cbutton.button);
			break;
		case SDL_KEYDOWN:
...
float Engine::Game::RandomNumber(float kecil, float besar) {

	float range = besar - kecil;
	
	return (kecil + (rand() * range) / (RAND_MAX + 1.0));
}
